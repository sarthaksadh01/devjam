...
import { getTest, getTestSubmission, saveTestSubmission, updateTestSubmission } from '../data/data';
...
class ViewTest extends React.Component {
    ...
    submitAns() {
        var submission = this.state.submission;
        updateTestSubmission(submission).then((doc) => {
        }).catch((err) => {
            NotificationManager.error("Error saving ans please check your internet connection..")
    ...
    onclickStartTest() {
        saveTestSubmission(this.state.submission).then((doc) => {
            this.setState({ submission: doc, isStarted: true });
